...
def loading_data():
    ...
    train_set = SHHB(cfg_data.DATA_PATH+'/train_simple', 'train_simple',main_transform=train_main_transform, img_transform=img_transform, gt_transform=gt_transform)
    train_loader = DataLoader(train_set, batch_size=cfg_data.TRAIN_BATCH_SIZE, num_workers=0, shuffle=True, drop_last=True)
    
    print('train data loaded')

    val_set = SHHB(cfg_data.DATA_PATH+'/test_simple', 'test_simple', main_transform=val_main_transform, img_transform=img_transform, gt_transform=gt_transform)
