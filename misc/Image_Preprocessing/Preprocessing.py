...
def split_den_map(number: int, n: int):
    with open(f"{number}.csv") as csv_file:
        ...
        for i in range(0, n):
            for j in range(0, n):
                split_maps.append([row[i * (width // n):(i + 1) * (width // n)] for row in
                                   den_map[j * (height // n):(j + 1) * (height // n)]])
                split_imgs.append([row[i * (width // n):(i + 1) * (width // n)] for row in
...
def blur_image(img: np.ndarray) -> np.ndarray:
    height, width = img.shape[0], img.shape[1]
    ran_blur = create_random_blur_kernel()
...
def split_and_process(number: int, split_n: int):
...
def create_random_blur_kernel() -> np.ndarray:
    ...
    for i in range(15):
        x = rand_inds[i][0]
        y = rand_inds[i][1]
        rand_x, rand_y = np.random.randint(x - 1 if x - 1 >= 0 else 0, x + 2 if x + 2 <= 14 else 14), np.random.randint(
...
def random_crop(img: np.ndarray, den: np.ndarray, size: int) -> (np.ndarray, np.ndarray):
    height, width = img.shape[0], img.shape[1]
    #print(f"{height}x{width}")
    ran_x, ran_y = np.random.randint(0, width-size), 0
    #print(ran_y, ran_x)
    if len(img) == 3:
        return img[ran_x:ran_x + size, ran_y:ran_y + size, :], den[ran_x:ran_x + size, ran_y:ran_y + size]
    else:
        return img[ran_x:ran_x + size, ran_y:ran_y + size], den[ran_x:ran_x + size, ran_y:ran_y + size]
